...
TNode **parse_lines(FILE *file){
  ...
  char *input, char_in, tmp;
   int iscommand, isdpdncy, line_end, file_end, empty_line, iscomment;    
   int line = 0, cnt, n_cnt = -1; // n_cnt: node count
   TNode *p_node, *c_node;
 
  // allocate memory for array of TNodes to be returned
   TNode **nodes = (TNode**)malloc(MAX_CHILDREN*sizeof(TNode*));
    ...
    // handles different cases for every char, and exit if there are error
      ...
      while(!line_end && !file_end && !empty_line){
         ...
         if(char_in == EOF){
			...
			if(iscomment && !cnt){
               	fprintf(stderr, "Line %d: Comment cannot be empty string\n", line);
				exit(1);
			}
            if(iscommand && !cnt){
         ...
         }else if(iscomment && char_in == '#'){
			fprintf(stderr, "Line %d: Comments contains '#' character\n", line);
			exit(1);
